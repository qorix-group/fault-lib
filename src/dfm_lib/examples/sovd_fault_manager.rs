...
fn main() {
    ...
    let storage = Arc::new(KvsSovdFaultStateStorage::new(storage_dir.path(), 0).unwrap());

    let cfg = load_config_file();
    let registry = Arc::new(FaultCatalogRegistry::new(vec![FaultCatalogBuilder::new().cfg_struct(cfg).build()]));

    let mut processor = FaultRecordProcessor::new(Arc::clone(&storage), Arc::clone(&registry));
